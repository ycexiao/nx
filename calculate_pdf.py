...
def get_pdf(structure_dict, element, u=0.007, rmax=10.0, qmax=30.0):
    ...
    diff_x_pdf = [x_pdf[0], list(np.array(x_pdf[1]) - np.array(diff_x_pdf[1]))]
    ...
    diff_n_pdf = [n_pdf[0], list(np.array(n_pdf[1]) - np.array(diff_n_pdf[1]))]
    return x_pdf, n_pdf, diff_x_pdf, diff_n_pdf


def sample_pdf(pdf, offset=0, force_length=None, 
                qmax=30, rmax=10, verbose=False):
    """
    ...
    expected_length_round = (expected_length // 10 -
                             expected_length // 100 * 10 + 1) * 10 + \
...
def add_pdf_to_datasets(load_name, save_name, element, force_length=100, u=0.007, rmax=10.0, qmax=30.0):
    """
    ...
    for doc in docs:
        ...
        for i in range(len(pdfs)):
            if i == 0:
                r, gr = sample_pdf(pdfs[i], force_length=force_length, qmax=qmax, rmax=rmax)
            else:
                _, gr = sample_pdf(pdfs[i], force_length=force_length, qmax=qmax, rmax=rmax)
            doc[names[i]] = gr
